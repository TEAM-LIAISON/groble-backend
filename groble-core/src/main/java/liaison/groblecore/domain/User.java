...
public class User extends BaseTimeEntity {
  ...
  /** 사용자 이메일 모든 계정 유형에 대해 중복없이 관리되는 이메일 */
  ...
  /**
   * 통합 계정(일반 회원가입) 유저 생성 메서드 이메일, 비밀번호를 받아 통합 계정 사용자 생성
   *
   * @param email 이메일 주소
  ...
  /**
   * OAuth2 소셜 로그인 유저 생성 메서드 소셜 로그인의 경우 별도의 이메일 인증 없이 바로 활성 상태로 생성
   *
   * @param email 이메일 주소
   * @param providerId 소셜 제공자 ID
   * @param providerType 소셜 제공자 유형 (GOOGLE, KAKAO, NAVER 등)
   * @return 생성된 User 객체
   */
  ...
  /** 로그인 시간 업데이트 (UTC 기준) 로그인할 때마다 호출되어 마지막 로그인 시간 업데이트 휴면 계정이 로그인하면 자동으로 활성화 */
  public void updateLoginTime() {
    this.lastLoginAt = Instant.now();

    // 만약 휴면 계정이 로그인하면 상태를 활성으로 변경
  ...
  public void confirmWithdrawal() {
    ...
    // 개인정보 비식별화 처리
    this.email = "withdrawn_" + this.id + "@example.com";
