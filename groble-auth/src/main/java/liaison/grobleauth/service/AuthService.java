...
/** 인증 서비스 회원가입, 로그인, 토큰 관리 등 인증 관련 기능 처리 */
@Slf4j
@Service
public class AuthService {
  ...
  public TokenResponse signup(SignupRequest request) {
    ...
    User user = User.createIntegratedUser(email, encodedPassword);
  ...
  public boolean changePassword(Long userId, String currentPassword, String newPassword) {
    ...
    // 현재 비밀번호 확인
    if (!passwordEncoder.matches(currentPassword, user.getIntegratedAccount().getPassword())) {
  ...
  private User createSocialUser(String email, String providerId, ProviderType providerType) {
    // 사용자 생성
    User user = User.createSocialUser(email, providerId, providerType);

    // 기본 사용자명 설정
    String defaultUsername = email.split("@")[0];
    user.updateUserName(defaultUsername);
