...
DROP TEMPORARY TABLE IF EXISTS tmp_mvs_agg;
CREATE TEMPORARY TABLE tmp_mvs_agg (
  ...
  PRIMARY KEY (market_id, stat_date, period_type)
) ENGINE=InnoDB;

INSERT INTO tmp_mvs_agg (
  market_id, stat_date, period_type,
...
FROM market_view_stats
GROUP BY market_id, stat_date, period_type;

-- 1) period_type 컬럼을 VARCHAR(16)로 변경 (ENUM 제거)
ALTER TABLE market_view_stats
...
DELETE FROM market_view_stats;


INSERT INTO market_view_stats (
  stat_date,
  market_id,
  created_at,
...
ALTER TABLE market_view_stats
  ADD CONSTRAINT uk_mvs_market_date_period
  UNIQUE (market_id, stat_date, period_type);
