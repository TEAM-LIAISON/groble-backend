...
//      } else {
//        log.warn("환불 실패 처리 시작 - 주문번호: {}, 실패사유: {}", merchantUid,
// refundResult.get("PCD_PAY_MSG"));
...
//          String.format(
//              "결제 완료된 주문만 취소할 수 있습니다. orderId: %d, status: %s", order.getId(),
// order.getStatus()));
//    }
//  }
//
...
//      throw new IllegalStateException(
//          String.format(
//              "결제 대기 상태의 주문만 처리할 수 있습니다. orderId=%d, status=%s", order.getId(),
// order.getStatus()));
//    }
//  }
//
...
//        throw new IllegalStateException(
//            String.format("결제 금액 불일치 - 주문금액: %s원, 결제금액: %s원", order.getFinalPrice(),
// paymentPrice));
//      }
...
//    log.info("페이플 승인 요청 - 주문번호: {}, authKey: {}", authResult.getPayOid(),
// authResult.getAuthKey());
//
...
//        throw new IllegalStateException(
//            String.format(
//                "복합과세 부가세 불일치 - DB: %s, 승인결과: %s", payment.getPcdPayTaxTotal(),
// approvedTaxTotal));
//      }
//    }
//
...
//
//    log.error(
//        "결제 취소 실패 - merchantUid: {}, errorCode: {}, message: {}", merchantUid, errorCode,
// errorMsg);
//
