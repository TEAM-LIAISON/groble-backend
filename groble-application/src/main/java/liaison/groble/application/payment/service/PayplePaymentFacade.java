...
// *
// * <p>회원/비회원 모두의 결제 및 결제 취소를 지원합니다.
// */
// @Slf4j
// @Service
...
//        (authInfo, approvalResult) -> transactionService.completePayment(authInfo,
// approvalResult),
//        this::publishPaymentCompletedEvent);
//  }
//
//  /**
//   * 결제 취소 처리
...
//   * 비회원 앱카드 결제 처리
//   *
//   * <p>비회원 사용자의 결제 인증 결과를 검증하고 승인 요청을 처리합니다.
//   *
...
//        guestUserId,
//        "비회원",
//        () -> transactionService.saveAuthAndValidateForGuest(guestUserId, paypleAuthResultDTO),
//        (authInfo, approvalResult) ->
//            transactionService.completePaymentForGuest(authInfo, approvalResult),
//        this::publishPaymentCompletedEventForGuest);
//  }
//
//  /**
//   * 비회원 결제 취소 처리
...
//
//    } catch (OrderNotFoundException | PaymentNotFoundException e) {
//      log.warn("비회원 결제 취소 실패 - 정보 없음: merchantUid={}", merchantUid);
//      throw e;
//    } catch (PaymentValidationException e) {
//      log.warn("비회원 결제 취소 검증 실패 - merchantUid={}, error={}", merchantUid, e.getMessage());
//      throw e;
//    } catch (PaypleApiException e) {
//      log.error("비회원 페이플 환불 API 호출 실패 - merchantUid={}", merchantUid, e);
//      throw new PaypleRefundException("비회원 환불 처리 중 오류가 발생했습니다", e);
//    } catch (Exception e) {
//      log.error("비회원 결제 취소 중 예상치 못한 오류 - merchantUid={}", merchantUid, e);
//      throw new PaymentProcessingException("비회원 결제 취소 처리 중 오류가 발생했습니다", e);
//    }
//  }
//
//  /** 비회원 결제 완료 이벤트 발행 */
//  private void publishPaymentCompletedEventForGuest(PaymentCompletionResult completionResult) {
//    log.info("=== 비회원 이벤트 발행 시작 === orderId: {}", completionResult.getOrderId());
//
//    try {
//      PaymentCompletedEvent event =
//          PaymentCompletedEvent.builder()
...
//              .build();
//
//      log.info("비회원 이벤트 객체 생성 완료: orderId={}", event.getOrderId());
//
//      eventPublisher.publish(event);
//
//      log.info("=== 비회원 이벤트 발행 완료 === orderId: {}", completionResult.getOrderId());
//
//    } catch (Exception e) {
//      log.error("비회원 이벤트 발행 중 예외 발생 - orderId: {}", completionResult.getOrderId(), e);
//      throw e;
//    }
//  }
//
//  /** 비회원 환불 완료 이벤트 발행 */
//  private void publishPaymentRefundedEventForGuest(PaymentCancelResult result) {
//    PaymentRefundedEvent event =
//        PaymentRefundedEvent.builder()
//            .orderId(result.getOrderId())
//            .paymentId(result.getPaymentId())
...
//      BiFunction<PaymentAuthInfo, PaypleApprovalResult, PaymentCompletionResult>
// completionFunction,
...
//      log.info("{} 결제창 닫힘 - {}, merchantUid: {}", userType, logId,
...
//            authInfo.getOrderId(), approvalResult.getErrorCode(),
// approvalResult.getErrorMessage());
//
//        throw new PayplePaymentApprovalException(
//            userType + " 결제 승인 실패: " + approvalResult.getErrorMessage());
//      }
//
//      // 5-2. 승인 성공 시 결제 완료 처리
//      PaymentCompletionResult completionResult = completionFunction.apply(authInfo,
// approvalResult);
//
//      // 6. 결제 완료 이벤트 발행
...
//      throw e;
//    } catch (PaypleApiException e) {
//      log.error("{} 페이플 API 호출 실패 - merchantUid: {}", userType, paypleAuthResultDTO.getPayOid(),
// e);
//      throw new PayplePaymentApprovalException(userType + " 결제 처리 중 오류가 발생했습니다", e);
//    } catch (Exception e) {
//      log.error(
//          "{} 결제 처리 중 예상치 못한 오류 - merchantUid: {}", userType, paypleAuthResultDTO.getPayOid(), e);
//      throw new PaymentProcessingException(userType + " 결제 처리 중 오류가 발생했습니다", e);
//    }
//  }
// }
